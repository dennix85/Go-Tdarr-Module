...
type CruddbRequest struct {
	// Allowed: ['SettingsGlobalJSONDB', 'LibrarySettingsJSONDB', 'NodeJSONDB', 'StatisticsJSONDB',
	// 'VariablesJSONDB', 'UsersJSONDB', 'ApiKeysJSONDB', 'FlowsJSONDB', 'StagedJSONDB',
	// 'FileJSONDB', 'F2FOutputJSONDB', 'WorkerVerdictHistoryJSONDB', 'JobsJSONDB', 'AutomationsJSONDB']
...
// Cruddb - For interacting with the database.
//
...
// DebugVarsType - For getting various debug info by type.
func (c *Client) DebugVarsType(debugType string) (json.RawMessage, error) {
	resp, err := c.get(fmt.Sprintf(c.baseURL+"/api/v2/debug-vars/%s", debugType))
	if err != nil {
		return nil, err
	}
	return c.decode(resp)
...
// DeleteFile - For deleting a file on disk from the Tdarr DB.
...
// DeleteUnhealthyFiles - For deleting files which have failed to transcode (table3) or are unhealthy (table6).
...
// IsServerAlive - Old endpoint for checking if the server is alive (use Status instead).
...
// SearchDb - Old endpoint for searching the file database (use ClientClienttype instead).
